...
api.fs.watchTree('', () => {
  $drives.set(api.fs.drives())
  $dirs.set([...$dirs.val])
})

const initpath = api.program.opts['file'] as string
$dirs.set(initpath?.split('/').slice(0, -1).map(p => p + '/') ?? ['usr/'])
...
const $breadcrumbs = $dirs.adapt(dirs =>
  dirs.map((part, idx) =>
    <api.Button padding={2} onClick={() => $dirs.set(dirs.slice(0, idx + 1))}>
      <api.Label text={part} />
    </api.Button>
  )
)
...
async function handleFile(path: string) {
  if (path.endsWith('.page.js')) {
    ...
    await import('/os/fs/' + path + '?decache=' + Date.now())
  }
  ...
  else if (path.endsWith('.bmp.jsln')) {
    await api.sys.launch('sys/apps/paint.app.js', path)
  }
